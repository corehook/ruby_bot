...
Telegram::Bot::Client.run(token) do |bot|
  bot.listen do |message |
    case message.text
      ...
      when '!list', '/list'
        Dir.glob("*.yml") { | file | bot.api.send_message(chat_id: message.chat.id, text: file, reply_markup: markup) }
      else
        if message.text.index("!deploy") == 0
          ...
          Dir.glob(a) { | file |
            ...
            result_log = `ansible-playbook #{file} -i hosts 2>&1`
            bot.api.send_message(chat_id: message.chat.id, text: result_log, reply_markup: markup)
          }
        else
          bot.api.send_message(chat_id: message.chat.id, text: "I didn't understand you:(, #{message.from.first_name} ")
        end
      end
  end
end
